...
class create_graph:
    def __init__(self, data_dir='dataset/Godess_final_data/Godess_data/', adj_dir='dataset/Godess_final_data/graph/directory_graph_connection/',
                 ...
                 use_atom=1, use_mono=1, use_ab=1, use_dl=1, use_pf=1, use_inter=1):
    ...
    def create_single_graph(self, f1, in_train_set, in_test_set):
        ...
        """
        embedding_matrix = torch.zeros([len(temp_df), len(self.atom_name_embed) * self.use_atom + \
                                        len(self.bound_ab_embed) * self.use_ab + \
                                        len(self.bound_dl_embed) * self.use_dl + len(self.carbon_pf_embed) * self.use_pf + \
                                        len(self.monosaccharide_simple_embed) * self.use_mono+ \
                                        len(self.me_embed) * self.use_inter + len(self.ser_embed) * self.use_inter + len(self.s_embed) * self.use_inter + \
                                        len(self.ac_embed) * self.use_inter + len(self.gc_embed) * self.use_inter], dtype=torch.float32)
