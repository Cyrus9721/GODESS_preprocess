...
class create_graph:
    def __init__(self, data_dir='dataset/Godess_final_data/Godess_data/', adj_dir='dataset/Godess_final_data/graph/directory_graph_connection/',
                 interaction_dir='dataset/Godess_final_data/graph/directory_graph_interaction/',
                 out_atom_name_embed='dataset/Godess_final_data/embedding/atom_name_embed.csv',
                 out_bound_orig_embed='dataset/Godess_final_data/embedding/bound_orig.csv',
                 out_atom_type_embed='dataset/Godess_final_data/embedding/atom_type.csv',
                 out_bound_AB_embed='dataset/Godess_final_data/embedding/bound_ab.csv',
                 out_DL_embed='dataset/Godess_final_data/embedding/bound_dl.csv',
                 out_PF_embed='dataset/Godess_final_data/embedding/carbon_pf.csv',
                 out_monosaccharide_accurate_embed='dataset/Godess_final_data/embedding/monosaccharide_accurate_embed.csv',
                 out_monosaccharide_simple_embed='dataset/Godess_final_data/embedding/monosaccharide_simple_embed.csv',
                 out_me_embed='dataset/Godess_final_data/embedding/root_me_embed.csv',
                 out_ser_embed='dataset/Godess_final_data/embedding/root_ser_embed.csv',
                 out_s_embed='dataset/Godess_final_data/embedding/component_s_embed.csv',
                 out_ac_embed='dataset/Godess_final_data/embedding/component_ac_embed.csv',
                 out_gc_embed='dataset/Godess_final_data/embedding/component_gc_embed.csv',
                 duplicate_mono_dict = 'dataset/Godess_final_data/mono_dictionary/dict_mono.json',
                 num_test=200, drop_threshold=5, additional_drop=[], seed=97211):
        ...
        self.duplicate_mono_dict = duplicate_mono_dict
    ...
    def create_single_graph(self, f1, in_train_set, in_test_set):
        ...
        """
        embedding_matrix = torch.zeros([len(temp_df), len(self.atom_name_embed) + len(self.bound_orig_embed) + \
                                        len(self.atom_type_embed) + len(self.bound_ab_embed) + \
                                        len(self.bound_dl_embed) + len(self.carbon_pf_embed) + \
                                        len(self.monosaccharide_accurate_embed) + len(self.monosaccharide_simple_embed) + \
        ...
        for i in range(len(temp_df)):
            ...
            embedding_matrix[i, :] = torch.tensor(np.concatenate([self.atom_name_embed[c_atom_name],
                                                                  self.bound_orig_embed[c_bound_orig],
                                                                  self.atom_type_embed[c_atom_type],
                                                                  self.bound_ab_embed[c_ab],
                                                                  self.bound_dl_embed[c_dl],
                                                                  self.carbon_pf_embed[c_pf],
                                                                  self.monosaccharide_accurate_embed[c_mono_accurate],
                                                                  self.monosaccharide_simple_embed[c_mono_simple],
                                                                  self.me_embed[str(c_me)],
                                                                  self.ser_embed[str(c_ser)],
                                                                  self.s_embed[str(c_s)],
                                                                  self.ac_embed[str(c_ac)],
                                                                  self.gc_embed[str(c_gc)]], axis=0))
        ...
        g.ndata['coordinate'] = torch.tensor(temp_df[['x', 'y', 'z']].values)
    ...
    def generate_train_test_split(self):
        ...
        # read in the duplicate mono file
        with open(self.duplicate_mono_dict, 'r') as f:
        ...
        for key in mono_dict:
            if len(mono_dict[key]) < self.drop_threshold:

                ignore_set.extend(mono_dict[key])

        ignore_idx_list = list(set(ignore_set))

        ignore_idx_list.extend(self.additional_drop)
        ...
        test_index = np.sort(np.random.choice(test_candidate_list, size=self.num_test, replace=False))
