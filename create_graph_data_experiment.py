...
class create_graph_experiment:
    def __init__(self, data_dir='data/directory_reformulate_combined/', adj_dir='graph/directory_graph_connection/',
                 ...
                 ab_embed_dir='experimental_data_combined/graph_combined/embedding/ab_embed.csv',
                 dl_embed_dir='experimental_data_combined/graph_combined/embedding/dl_embed.csv',
                 pf_embed_dir='experimental_data_combined/graph_combined/embedding/pf_embed.csv',
                 record_index='experimental_data_combined/graph_index_outlier_check/',
    ...
    def create_single_graph(self, f1, in_train_set, in_test_set, graph_index):
        ...
        embedding_matrix = torch.zeros([len(temp_df), len(self.atom_embed) + len(self.mono_embed) + \
                                        len(self.ab_embed) + len(self.dl_embed) + len(self.pf_embed)],
                                       dtype=torch.float32)
        ...
        for i in range(len(temp_df)):
            ...
            embedding_matrix[i, :] = torch.tensor(np.concatenate([self.atom_embed[c_atom],
                                                                  self.mono_embed[c_mono],
                                                                  self.ab_embed[c_ab],
                                                                  self.dl_embed[c_dl],
                                                                  self.pf_embed[c_pf]], axis=0))
    ...
    def create_all_graph(self):
        ...
        df_index_outlier_check = pd.DataFrame([range(len(self.files_labels_list)),
                                               self.files_labels_list]).T.to_csv(os.path.join(self.record_index,
                                                                                              'graph_index_outlier_check.csv'),
                                                                                 index=False)
